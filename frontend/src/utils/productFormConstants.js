...
export const normalizeSkinTypes = (values = []) => {
  if (!Array.isArray(values)) return [];

  return [...new Set(
    values
      .map((value) => String(value || '').trim().toLowerCase())
      .filter((value) => SKIN_TYPES.includes(value))
  )];
};
...
export const SUGGESTED_RETAILERS = [
  'Amazon',
  'Nykaa',
  'Myntra',
  'Flipkart',
...
export const toEditorImages = (images = []) => {
  ...
  if (list.length && !list.some((image) => image.isPrimary)) {
...
export const offersToForm = (offers = []) =>
  (offers || []).map((offer) => ({
    localId: String(
      offer._id || `opt-${Math.random().toString(36).slice(2, 7)}`
    ),
    _id: offer._id || null,
...
export const normalizeProductData = (extracted = {}, options = {}) => {
  ...
  return {
    ...
    slug:
      extracted.slug ||
      (extracted.name || '').toLowerCase().replace(/\s+/g, '-'),
    brand: extracted.brand || '',
    category: extracted.category || '',
    variant: extracted.variant || '',
    size: extracted.size || '',
    quantity: extracted.quantity || '',
    claims: Array.isArray(extracted.claims)
    ...
    keyIngredients: parseFormIngredientList(
      extracted.keyIngredients
    ).join(', '),
    ...
    // FIX:
    // Convert Combination/Dry/Oily/Sensitive/Normal
    // into the lowercase values required by Product.js
    skinTypes: normalizeSkinTypes(extracted.skinTypes),
    ...
    essentialOilFree: formatTriState(
      extracted.essentialOilFree ?? null
    ),

    pregnancyFriendly: formatTriState(
      extracted.pregnancyFriendly ?? null
    ),

    purchaseOptions: purchaseOptionFromExtraction(
      extracted,
      sourceUrl
    ),
...
export const mergeExtractedIntoForm = (
  currentForm,
  extracted = {},
  options = {}
) => {
  ...
  const keepIngredients =
    currentForm?.ingredientsTouched &&
    currentForm?.ingredients?.trim() &&
    !replaceIngredients;
  ...
  incoming.images.forEach((image) => {
    if (!mergedImages.some((entry) => entry.url === image.url)) {
      mergedImages.push({
        ...image,
        isPrimary: false,
      });
  ...
  if (
    mergedImages.length &&
    !mergedImages.some((image) => image.isPrimary)
  ) {
  ...
  incoming.purchaseOptions.forEach((option) => {
    const match = mergedOptions.find(
      (entry) =>
        entry.retailer &&
        option.retailer &&
        entry.retailer.toLowerCase() ===
          option.retailer.toLowerCase()
    );

    if (match) {
      if (!match.productUrl) {
        match.productUrl = option.productUrl;
      }
  ...
  return {
    ...
    ingredientsTouched: keepIngredients
      ? true
      : incoming.ingredientsTouched,

    sourceUrl:
      incoming.sourceUrl ||
      currentForm.sourceUrl ||
      '',
...
export const extractedDataToForm = (
  extracted = {},
  sourceUrl = ''
) =>
  normalizeProductData(extracted, { sourceUrl });

export const productToForm = (
  product = {},
  offers = []
) => ({
  name: product.name || '',
  ...
  // Normalize existing database/API values too
  skinTypes: normalizeSkinTypes(product.skinTypes),
...
export const formToExtractedData = (form) => ({
  ...
  skinTypes: normalizeSkinTypes(form.skinTypes),
  ...
  price:
    form.purchaseOptions?.[0]?.price !== '' &&
    form.purchaseOptions?.[0]?.price != null
      ? Number(form.purchaseOptions[0].price)
      : null,

  retailer:
    form.purchaseOptions?.[0]?.retailer?.trim() || '',
});
...
export const validateProductForm = (form) => {
  ...
  (form.purchaseOptions || []).forEach((option, index) => {
    ...
    if (!option.retailer?.trim()) {
      errors[`purchaseOptions.${index}.retailer`] =
    ...
    if (
      ...
      errors[`purchaseOptions.${index}.price`] =
        'Enter a numeric price.';
    }

    if (!option.productUrl?.trim()) {
      errors[`purchaseOptions.${index}.productUrl`] =
        'Product URL is required.';
    }

    if (
      option.isActive &&
      !option.affiliateUrl?.trim()
    ) {
      errors[`purchaseOptions.${index}.affiliateUrl`] =
        'Affiliate URL is required for an active option.';
    }
  });
...
export const formToSavePayload = (form) => ({
  ...
  ingredients: parseFormIngredientList(
    form.ingredients
  ),
  ...
  // Final safety normalization before saving
  skinTypes: normalizeSkinTypes(form.skinTypes),

  concerns: form.concerns,

  fragranceFree: parseTriState(
    form.fragranceFree
  ),

  alcoholFree: parseTriState(
    form.alcoholFree
  ),

  essentialOilFree: parseTriState(
    form.essentialOilFree
  ),

  pregnancyFriendly: parseTriState(
    form.pregnancyFriendly
  ),
